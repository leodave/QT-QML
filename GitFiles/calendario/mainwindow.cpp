...

{
  ...
  //  QDate fechaActual = QDate::currentDate();
   ...
   // qDebug() << fechaActual.;
    ...
    for(int i = 0; i < (incioDeMes.daysTo(finalDeMes) + 1); i ++){
...
//        qDebug() << incioDeMes.addDays(dia);
        ...
        connect(days[i],SIGNAL(diaClickeado(int)),this,
                 SLOT(diaSeleccionado(int)));
//        connect(days[dia], SIGNAL(diaHovered(QVector<QString>,int)),this,
//                SLOT(actualizarLabelEventos(int,int)));
        days[i]->resize(100,100);
        days[i]->setFecha(incioDeMes.addDays(i).day());
        days[i]->move((i%7) * DIMENSION_DIAS, (i/7) * DIMENSION_DIAS);
    }
